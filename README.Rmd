...
Matrix multiplication is also defined through inner products of vectors.  For two vectors $\textbf{x}$ and $\textbf{y}$ of the same length, their inner product is $\bigoplus$ $\textbf{x}'\textbf{y} = \bigoplus_{k=1}^{n} x_{k} \odot y_{k} = \min_{k = 1,\ldots,n}\{x_{k}+y_{k}\}$.  In __tropical__, this is given by `%..%` (which is the tropical analogue of R's ordinary matrix multiplication operator `%*%`):
```{r inner-product}
1:3 %..% 4:6
```
