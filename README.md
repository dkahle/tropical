...
Matrix multiplication is also defined through inner products of vectors. For two vectors **x** and **y** of the same length, their inner product is $\\bigoplus\_{k=1}^{n} x\_{k} \\odot y\_{k}$ $\\textbf{x}'\\textbf{y} = \\bigoplus\_{k=1}^{n} x\_{k} \\odot y\_{k} = \\min\_{k = 1,\\ldots,n}\\{x\_{k}+y\_{k}\\}$. In **tropical**, this is given by `%..%` (which is the tropical analogue of R's ordinary matrix multiplication operator `%*%`):
